...
/*
dtapped_istream - double tapped istream class
This header file defines a child class of std::istream
...
#pragma warning(default:4996)
		...
		~basic_dtapped_streambuf(){tap.close();std::remove(final);std::rename(temp,final);}
	...
	public:
		basic_dtapped_istream(const elem* tempfname,const elem* finalfname,
			std::basic_streambuf<elem,traits>* sb,std::basic_ostream<elem,traits>* os=NULL):
		  std::basic_istream<elem,traits>(&buf),buf(tempfname,finalfname,sb){tie(os);}
...
#define TAPPED_FINAL_WFILE TAPPED_FINAL_TFILE
	...
	dtapped_istream dtp_cin(TAPPED_TEMP_FILE,TAPPED_FINAL_FILE,std::cin.rdbuf(),&std::cout);
#endif	//defined(TAPPED_TEMP_FILE) && defined(TAPPED_FINAL_FILE)

#if defined(TAPPED_TEMP_WFILE) && defined(TAPPED_FINAL_WFILE)
	dtapped_wistream dtp_wcin(TAPPED_TEMP_WFILE,TAPPED_FINAL_WFILE,std::wcin.rdbuf(),&std::wcout);
